...
namespace CafeteriaManagementSystemServer.Repository
{
    ...
    public class RecommendationRepository
    {
        ...
        public List<string> getRecommendation(RecommendatioData data)
        {
    ...
    .ThenByDescending(g => g.AverageSentimentScore)
    .Take(data.NumberOfDishes)
    .Select(g => g.MenuId) // Select only the MenuId
    .ToList();
            List<string> menuList = new List<string>();
            foreach ( int topDishMenuId in topDishMenuIds )
            {
                
                FoodItem food = _DbContext.FoodItems.Where(x=>x.Id == topDishMenuId).FirstOrDefault();
                menuList.Add(food.Name);
            }
            return menuList;
        }
    }
