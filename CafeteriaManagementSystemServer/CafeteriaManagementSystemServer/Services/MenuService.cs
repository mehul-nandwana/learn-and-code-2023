...
namespace CafeteriaManagementSystemServer.Services
{
    public class MenuService : IMenuService
    {
        ...
        public Response AddMenu(int[] menuData)
        {
            ...
            {
                return new Response(ex.Message, menuData, Constant.USER_LOGIN);
            }
        }
        ...
        public Response GetMenu(int userId)
        {
            try
            {
                ...
                var foodItemData = MapToFoodItemData(foodItems);

                return new Response(Constant.SUCCESS_MESSAGE, foodItemData, Constant.SHOW_MENU);
            }
            catch (Exception ex)
            {
        ...
        public Response AddChoiceForMenu(ChoiceData choiceData)
        {
            try
            {
                ...
                var status = _menuRepository.AddChoice(choice);

                return new Response(status, choiceData.UserId, Constant.EMPLOYEE_LOGIN);
            }
            catch (Exception ex)
            {
                return new Response(ex.Message, Constant.EMPTY_STRING, Constant.USER_LOGIN);
        ...
        private List<FoodItemData> MapToFoodItemData(List<FoodItem> foods)
        {
            var foodItemDataList = new List<FoodItemData>();
