...
namespace CafeteriaManagementSystemServer.Services
{
    public class MenuService: IMenuService
    {
        ...
        public Response GetMenu()
        {
            ...
            try
            {
                List<int> foodIds = menuRepository.GetMenu();
                List<FoodItem> foods = foodRepository.GetFoodItems(foodIds);
               List< FoodItemData> foodItemData = SetFoodData(foods);
                response = new Response(Constant.SUCCESS_MESSAGE, foods, Constant.SHOW_MENU);
                return response;
            }
            catch (Exception ex)
            {
        ...
        public Response AddChoiceForMenu(ChoiceData choiceData)
        {
            ...
            try
            {
                ...
                menuRepository.AddChoice(choice);
                response = new Response(Constant.SUCCESS_ADD_CHOICE, choiceData, Constant.EMPLOYEE_LOGIN);
            }
            catch(Exception ex)
            {
                response = new Response(ex.Message, Constant.EMPTY_STRING, Constant.USER_LOGIN);
