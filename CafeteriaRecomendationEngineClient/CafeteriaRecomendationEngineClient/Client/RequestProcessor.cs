...
namespace CafeteriaRecomendationEngineClient.Client
{
    public class RequestProcessor
    {
        ...
        private CustomProtocolParameters CreateAddFeedbackRequest(CustomProtocolParameters serializedRequest)
        {
            ...
            NotificationData notificationData = _jsonSerializer.DeserializeObject<NotificationData>(serializedRequest.Obj);
            List<string> notifications = notificationData.NotificationMessage;
            int userId = notificationData.userId;
            if (notifications.Count != 0)
            {
                FeedBackData feedbackData = _userInput.GetFeedbackData(notificationData);
